...
def compute_difference(pre_data_list, post_data_list, initial, post):
    ...
    result = math.sqrt(
        math.pow(x_old - x_new, 2)
        + math.pow(y_old - y_new, 2)
        + math.pow(z_old - z_new, 2)
    )
    return result
...
class ForceSensorCapture(object):
    ...
    def __init__(self):
        ...
        self.wrist_roll_sub = rospy.Subscriber(
            "/hsrb/joint_states", JointState, self.__angle_cb
        )

        # Wait for connection
        try:
            rospy.wait_for_message(
                ft_sensor_topic, WrenchStamped, timeout=_CONNECTION_TIMEOUT
            )
...
def calculate_force():
    ...
    force_difference = compute_difference(
        pre_force_list, post_force_list, pre_angle, post_angle
    )
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(5):
            ...
            force_angle = get_max_directional_force(
                post_force_list[0][0], post_force_list[1][0], post_force_list[2][0]
            )
